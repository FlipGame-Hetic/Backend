# UC-04 : Lancer et jouer une partie PvE

| ID    | Nom                   | Acteur | But                                     |
| ----- | --------------------- | ------ | --------------------------------------- |
| UC-04 | Lancer une partie PvE | Joueur | Démarrer une run solo contre les 3 boss + mode Endless |

### UC-04 : Lancer et jouer une partie PvE (détaillé)

**Acteurs :** Joueur (via boutons de navigation, flippers, plunger, boutons d'ultime et gyroscope)

**Préconditions :** Le joueur a sélectionné son personnage (UC-03), le mode PvE est choisi, les 3 écrans sont synchronisés, le serveur est joignable
...
- 3a. Le joueur perd sa bille → UC-07 déclenché (perte de vie)
- 4a. La barre d'ultime est pleine → le joueur peut déclencher UC-06 (ultime)
- 5a. Le joueur agite la borne → UC-09 déclenché (tilt)
